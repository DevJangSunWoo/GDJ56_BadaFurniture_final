	...
	public static String getAlertMsg(String path, String state, Object obj) {
		...
		try {
			...
			case "resellComment":
				...
				msg = "<a href=\"" + path + "/resell/read.do?resellNo=" + resell.getResellNo() + "\">"
						+ resell.getItem() + " 판매신청(" + formatter.format(resell.getHopePrice()) + ")"
						+ "</a>에 운영자가 댓글을 달았습니다.";
				break;
			// 내 가구 팔기 판매 상태가 변경되었을 때
			case "progressState":
				Resell updateResell = (Resell) obj;
				msg = "<a href=\"" + path + "/resell/read.do?resellNo=" + updateResell.getResellNo() + "\">"
						+ updateResell.getItem() + " 판매신청(" + formatter.format(updateResell.getHopePrice()) + ")"
						+ "</a>" + "의 판매상태가 <b>" + updateResell.getProgressState() + "</b>"
			...
			case "refundState":
				...
				if (orderDetail.getRefundState() != null) {
					...
					msg = "<a href=\"" + path + "/mypage/refund.do?orderDetailNo=" + orderDetail.getOrderDetailNo()
							+ "\">[" + refundState + "]" + orderDetail.getProduct().getTitle() + "("
							+ formatter.format(orderDetail.getProduct().getPrice()) + ")" + "</a>의 " + refundState
							+ "상태가 <b>" + orderDetail.getRefundState() + "</b>로 변경되었습니다.";
				}
				break;
			case "undeposited":
				...
				msg = "<a href=\"" + path + "/mypage/order.do?orderSheetNo=" + uos.getOrderSheetNo() + "\">" + "주문번호 "
						+ uos.getOrderSheetNo() + " 구매신청(" + formatter.format(uos.getTotalPrice()) + ")" + "</a>이"
			...
			e.printStackTrace();
			throw new RuntimeException("AlarmMsg 만들기 실패");
		}
		return msg;
	}
}
