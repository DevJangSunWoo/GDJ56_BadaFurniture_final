...
function onlyKorean(obj) {
	if(event.keyCode != 8 && event.keyCode != 46 && (event.keyCode < 12592) || (event.keyCode > 12687)){
		event.returnValue = false
	}
}
/*
...
/*
	공지사항 레이어 팝업 출력
*/
function popup_layer(div_id, ns_no, idx, top_id, top_val, left_val) {

	//if(popup_seq < 3){
	if(top_id && top_id != ""){
		var top_obj = document.getElementById(top_id);
		var top = top_obj.offsetTop + top_val;
		var left = top_obj.offsetLeft + left_val;
	} else {
		var top = 100;
	...
	if ( $("#" + div_id).length == 0 )
	{
		$("<div id='" + div_id + "' style='width: 500px; position: absolute; left: " + left + "px; top:" + top + "px; z-index: " + z_index + "; display: none; background-color: #ffffff;'></div>").appendTo("body");
	}
...
해당 폼 감추기
*/
function display_form(obj1,obj2,obj3){

	var result = (typeof obj3 !== 'undefined');
 ...
 * 로컬브라우저 데이터 저장
 */
var setBrowserData = function(key,val)
{
 ...
 * 로컬브라우저 데이터 취득
 */
var getBrowserData = function(key)
{
	if(window.localStorage)
	{
...
// 금지어 체크
function chkBlackKeyword(txt, obj) {

	var isPass = true;

	$.ajax({
		type : 'POST',
		url : '/app/api/cs/chk_black_keyword/',
		data : {
			'txt' : txt
		},
		async: false,
...
function ceil(n, pos) {
	...
	var num = Math.ceil(Math.floor(n * digits) / digits);
